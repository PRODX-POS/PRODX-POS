 ...
 * infrastructure / backend API layer.
 */

import { User, SessionContext, Store } from '../domain/auth';
import {
  Product, Category, InventoryLedgerEntry, StockMovementReason, BulkImportItem,
  BulkImportMode, BulkImportResult, BatchPriceAdjustmentParams, BatchPriceAdjustmentResult,
} from '../domain/catalog';
...
export interface CheckoutRequest {
  idempotencyKey: string; storeId: string; registerId: string; cashierId: string;
  customer?: { id: string; name: string; phone: string; email: string };
  items: readonly CartLineItem[]; totals: CartTotals; payments: readonly TenderPayment[];
  notes?: string; isOfflineSubmission?: boolean;
}
export interface CheckoutResponse { readonly success: boolean; readonly order: Order; readonly serverConfirmed: boolean; readonly message: string; readonly idempotencyCached?: boolean; }
...
export interface ICatalogApi {
  ...
  adjustStock(storeId: string, productId: string, quantityDelta: number, reason: StockMovementReason, userId: string, notes?: string): Promise<InventoryLedgerEntry>;
  bulkAdjustStock(storeId: string, productIds: readonly string[], quantityDelta: number, reason: StockMovementReason, userId: string, notes?: string): Promise<readonly InventoryLedgerEntry[]>;
  ...
  batchPriceAdjustment(params: BatchPriceAdjustmentParams): Promise<BatchPriceAdjustmentResult>;
  bulkImportProducts(storeId: string, items: readonly BulkImportItem[], mode: BulkImportMode, userId: string, notes?: string): Promise<BulkImportResult>;
}
export interface RefundItemRestock { productId: string; quantity: number; }
export interface IOrderApi {
  ...
  refundOrder(storeId: string, orderId: string, refundAmount: Money, reason: string, refundMethod: 'cash' | 'card' | 'qr_digital', restockItems: boolean, authorizedByUserId: string, authorizedByName: string, itemsToRestock?: readonly RefundItemRestock[]): Promise<Order>;
}
export interface IShiftApi {
  ...
  recordCashMovement(shiftId: string, type: CashMovementType, amount: Money, reason: string, userId: string): Promise<CashMovement>;
...
export interface IAuditApi {
  recordEvent(storeId: string, registerId: string, userId: string, userName: string, action: AuditAction, severity: AuditSeverity, details: Record<string, unknown>): Promise<AuditLogEntry>;
