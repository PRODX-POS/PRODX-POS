...
export const Drawer: React.FC<DrawerProps> = ({
  ...
  return createPortal(
    <div
      id={id}
      className="fixed inset-0 z-50 overflow-hidden bg-black/60 dark:bg-black/80 backdrop-blur-sm transition-opacity animate-in fade-in duration-150"
      onClick={(e) => {
    ...
    >
      <div className={`fixed inset-y-0 ${side === 'right' ? 'right-0' : 'left-0'} flex max-w-full`}>
        <div
          className={`${width} bg-card border-crisp border-l border-border flex flex-col h-full shadow-xl animate-in slide-in-from-${
        ...
        >
          <div className="flex items-center justify-between px-6 py-4.5 border-b border-crisp border-border shrink-0 bg-card">
            <div>
              ...
              {description && (
                <p className="mt-0.5 text-xs text-text/70">{description}</p>
              )}
            </div>
            <button
              type="button"
