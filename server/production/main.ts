...
const parsePort = (raw: string | undefined): number => {
  ...
  if (!Number.isInteger(value) || value < 1 || value > 65535) throw new Error('PORT must be an integer between 1 and 65535.');
  return value;
};

const parsePositiveIntegerEnv = (name: string, raw: string | undefined, fallback: number): number => {
    ...
    configurePublicRoutes: (configuredApp) => {
      configuredApp.post('/api/v1/auth/login', async (request, response) => {
        ...
        if (typeof username !== 'string' || typeof password !== 'string' || typeof deviceId !== 'string') {
          response.status(400).json({ error: { code: 'INVALID_REQUEST', message: 'username, password, and deviceId are required.', requestId: request.id } });
          return;
        }
        const result = await authentication.authenticateCredentials({ username, password, deviceId });
        if (!result) {
          response.status(401).json({ error: { code: 'INVALID_CREDENTIALS', message: 'Credentials or device authorization are invalid.', requestId: request.id } });
          return;
        }
