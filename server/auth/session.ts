...
export type AuthenticatedSession = RequestPrincipal & { sessionId: string; expiresAt: Date; deviceId: string };
export type CredentialRecord = {
  userId: string; organizationId: string; username: string; status: 'active' | 'disabled'; credentialType: 'password'; secretHash: string; failedAttempts: number; lockedUntil: Date | null;
};
export type SessionRecord = {
  id: string; organizationId: string; userId: string; deviceId: string; tokenHash: string; expiresAt: Date; revokedAt: Date | null; userStatus: 'active' | 'disabled';
};
...
export const createSessionIssuer = (repository: AuthenticationRepository, verifySecret: (secret: string, encodedHash: string) => Promise<boolean>, now: () => Date = () => new Date()): SessionIssuer => ({
  ...
  authenticateBearer: async (token) => {
    ...
    return { sessionId: session.id, userId: session.userId, organizationId: session.organizationId, storeId: device.storeId, deviceId: device.id, expiresAt: session.expiresAt };
  },
