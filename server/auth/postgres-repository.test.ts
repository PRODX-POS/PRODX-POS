...
test('credential lookup maps authoritative user and credential state within the requested organization', async () => {
  const calls: Array<{ sql: string; parameters: readonly unknown[] }> = [];
  const db: SqlExecutor = {
    async query<T extends Record<string, unknown>>(sql: string, parameters = []): Promise<readonly T[]> {
      calls.push({ sql, parameters });
      const row = { userId: 'user-1', organizationId: 'org-1', username: 'cashier', status: 'active', credential_type: 'password', secret_hash: 'scrypt$hash', failed_attempts: 2, locked_until: null };
  ...
  assert.deepEqual(result, { userId: 'user-1', organizationId: 'org-1', username: 'cashier', status: 'active', credentialType: 'password', secretHash: 'scrypt$hash', failedAttempts: 2, lockedUntil: null });
...
test('session lookup joins user status instead of trusting session state', async () => {
  const db: SqlExecutor = { async query<T extends Record<string, unknown>>(): Promise<readonly T[]> {
    return [{ id: 'session-1', organization_id: 'org-1', user_id: 'user-1', device_id: 'device-1', token_hash: 'hash', expires_at: new Date('2030-01-01T00:00:00Z'), revoked_at: null, user_status: 'active' } as unknown as T];
  }};
...
test('credential and session writes remain parameterized and persist auth security events atomically', async () => {
  const calls: Array<{ sql: string; parameters: readonly unknown[] }> = [];
  const db: SqlExecutor = { async query<T extends Record<string, unknown>>(sql: string, parameters = []): Promise<readonly T[]> { calls.push({ sql, parameters }); return []; }};
  const repository = createPostgresAuthenticationRepository(db);
  ...
  await repository.revokeSession('session-1', new Date('2030-01-01T00:05:00Z'));
  ...
  assert.match(calls[4].sql, /revoked_at/);
});
