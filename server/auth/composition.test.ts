...
const makeDb = (passwordHash: string, calls: string[]): SqlExecutor => ({
  async query<T extends Record<string, unknown>>(sql: string, parameters = []): Promise<readonly T[]> {
    ...
    if (sql.includes('FROM prodx_users u')) {
      const row = { userId: 'user-1', organizationId: 'org-1', username: 'cashier', status: 'active', credential_type: 'password', secret_hash: passwordHash, failed_attempts: 0, locked_until: null };
      return [row as unknown as T];
    }
    if (sql.includes('FROM prodx_devices')) return [{ id: 'device-1', organization_id: 'org-1', store_id: 'store-1', status: 'active' } as unknown as T];
    return [];
  },
});
...
test('application composition builds authentication against the injected PostgreSQL boundary', async () => {
  ...
  const result = await authentication.authenticateCredentials({ username: 'cashier', password: 'correct-password', deviceId: 'device-1', organizationId: 'org-1' });
  assert.ok(result);
  ...
  assert.equal(calls.some((call) => call.includes('u.organization_id = $2')), true);
});
