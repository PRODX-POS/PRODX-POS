...
test('wrong credentials do not create sessions and record a failed attempt', async () => {
  ...
  assert.equal(await issuer.authenticateCredentials({ username: 'cashier', password: 'bad', deviceId: device.id }), null); assert.equal(getSession(), null); assert.equal(calls.failed, 1); assert.equal(calls.lastLockedUntil, null);
});
...
test('five failed attempts lock the credential for fifteen minutes and lockout attempts do not increment', async () => {
  ...
  const issuer = createSessionIssuer(repository, async () => false, () => clock);
  for (let attempt = 1; attempt <= 5; attempt += 1) assert.equal(await issuer.authenticateCredentials({ username: 'cashier', password: 'bad', deviceId: device.id }), null);
  assert.equal(getCredential().failedAttempts, 5); assert.deepEqual(calls.lastLockedUntil, new Date('2026-09-13T12:15:00.000Z'));
  assert.equal(await issuer.authenticateCredentials({ username: 'cashier', password: 'bad', deviceId: device.id }), null); assert.equal(calls.failed, 5); assert.equal(getCredential().failedAttempts, 5);
});
...
test('successful authentication resets failed attempts and clears lockout state', async () => {
  ...
  for (let attempt = 0; attempt < 4; attempt += 1) assert.equal(await issuer.authenticateCredentials({ username: 'cashier', password: 'bad', deviceId: device.id }), null);
  assert.equal(getCredential().failedAttempts, 4); const result = await issuer.authenticateCredentials({ username: 'cashier', password: 'good', deviceId: device.id }); assert.ok(result); assert.equal(getCredential().failedAttempts, 0); assert.equal(getCredential().lockedUntil, null);
});
...
test('disabled device, locked user, and cross-tenant device cannot authenticate', async () => {
  ...
  assert.equal(await issuer.authenticateCredentials({ username: 'cashier', password: 'good', deviceId: device.id }), null); repository.findCredentialByUsername = async () => credential;
  repository.findDevice = async () => ({ ...device, organizationId: '00000000-0000-0000-0000-000000000099' }); assert.equal(await issuer.authenticateCredentials({ username: 'cashier', password: 'good', deviceId: device.id }), null);
});
...
test('bearer authentication rejects expired, revoked, disabled-user, and disabled-device sessions', async () => {
  ...
  const result = await issuer.authenticateCredentials({ username: 'cashier', password: 'good', deviceId: device.id }); assert.ok(result);
