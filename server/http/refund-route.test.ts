...
test('refund route derives the authorizing user from the authenticated principal', async () => {
  ...
  const query = async <T extends QueryResultRow>(sql: string, parameters: readonly unknown[] = []) => {
        if (sql.includes('FROM prodx_refunds r JOIN')) return { rows: [] as unknown as T[] };
        ...
        if (sql.includes('FROM prodx_payments')) return { rows: [{ amount: '10.00', mismatched_currency_count: 0 }] as unknown as T[] };
        if (sql.includes('SELECT COALESCE(SUM(amount)')) return { rows: [{ amount: '0.00' }] as unknown as T[] };
        ...
        if (sql.startsWith('INSERT INTO prodx_refunds')) {
          ...
          return { rows: [{ id: 'refund-1', order_id: 'order-1', amount: '10.00' }] as unknown as T[] };
        }
