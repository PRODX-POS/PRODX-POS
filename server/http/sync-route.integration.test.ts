...
const command = (commandId: string, key: string, payload = checkoutRequest(key)) => ({ commandId, type: 'order_transaction', idempotencyKey: key, payload });
...
async function post(baseUrl: string, body: unknown): Promise<Response> {
  return fetch(`${baseUrl}/api/v1/sync/commands`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(body) });
}
...
test('PostgreSQL sync command lifecycle is terminal, replay-safe and conflict-safe', async (t) => {
  ...
  assert.equal((await post(baseUrl, command('cmd-complete', 'sync-complete'))).status, 201);
  assert.equal((await post(baseUrl, command('cmd-complete', 'sync-complete'))).status, 200);
  assert.equal((await pool.query('SELECT count(*)::int AS n FROM prodx_orders WHERE idempotency_key=$1', ['sync-complete'])).rows[0].n, 1);

  assert.equal((await post(baseUrl, command('cmd-complete', 'sync-other'))).status, 409);
  ...
  assert.equal((await post(baseUrl, command('cmd-failed', 'sync-failed'))).status, 409);
  ...
  assert.equal((await post(baseUrl, command('cmd-failed', 'sync-failed'))).status, 201);
