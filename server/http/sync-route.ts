...
type SyncCommand = { commandId: string; type: 'order_transaction'; idempotencyKey: string; payload: CheckoutRequest };
...
const parseCommand = (value: unknown): SyncCommand => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new CheckoutValidationError('Sync command must be an object.');
  const v = value as Record<string, unknown>;
  if (typeof v.commandId !== 'string' || !v.commandId.trim()) throw new CheckoutValidationError('commandId is required.');
  if (v.type !== 'order_transaction') throw new CheckoutValidationError('Unsupported sync command type.');
  if (typeof v.idempotencyKey !== 'string' || !v.idempotencyKey.trim()) throw new CheckoutValidationError('idempotencyKey is required.');
  if (!v.payload || typeof v.payload !== 'object' || Array.isArray(v.payload)) throw new CheckoutValidationError('payload must be an object.');
  return { commandId: v.commandId.trim(), type: 'order_transaction', idempotencyKey: v.idempotencyKey.trim(), payload: v.payload as CheckoutRequest };
};
...
export const registerSyncRoute = (app: Express, db: TransactionalSqlExecutor, permission = 'pos.sell'): void => {
  ...
  app.post('/api/v1/sync/commands', requirePermission(permission), async (request: Request, response: Response) => {
    try {
      ...
      if (command.payload.storeId !== context.principal.storeId || command.payload.cashierId !== context.principal.userId || command.payload.idempotencyKey !== command.idempotencyKey) {
        response.status(403).json({ error: { code: 'STORE_SCOPE_VIOLATION', message: 'Sync payload must match the authenticated principal and command idempotency key.', requestId: request.id } }); return;
      }
