...
test('login is explicitly public and returns issued session',async()=>{const authentication:SessionIssuer={authenticateCredentials:async()=>({token:'token',sessionId:'session'}),authenticateBearer:async()=>null};const app=createApp({authenticateRequest:async()=>null,publicPaths:['/api/v1/auth/login'],configureRoutes:configuredApp=>registerAuthRoute(configuredApp,authentication)});const response=await requestJson(app,{username:'cashier',password:'secret',deviceId:'device'});assert.equal(response.status,200);assert.deepEqual(await response.json(),{token:'token',sessionId:'session'});});

test('failed login does not disclose credential details',async()=>{const authentication:SessionIssuer={authenticateCredentials:async()=>null,authenticateBearer:async()=>null};const app=createApp({authenticateRequest:async()=>null,publicPaths:['/api/v1/auth/login'],configureRoutes:configuredApp=>registerAuthRoute(configuredApp,authentication)});const response=await requestJson(app,{username:'cashier',password:'wrong',deviceId:'device'});assert.equal(response.status,401);assert.equal((await response.json()).error.code,'INVALID_CREDENTIALS');});
