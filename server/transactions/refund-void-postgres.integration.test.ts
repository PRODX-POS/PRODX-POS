...
test('PostgreSQL refund/void are authoritative, idempotent, atomic and bounded',async t=>{
  ...
  await assert.rejects(adjustments.refund({storeId:id.store,orderId:sale.order.id,amountInCents:1000,currency:'THB',reason:'Customer returned one unit',refundMethod:'cash',authorizedByUserId:id.user,idempotencyKey:'refund-1',itemsToRestock:[]}));
  ...
  await assert.rejects(adjustments.refund({storeId:id.store,orderId:sale.order.id,amountInCents:500,currency:'THB',reason:'Mismatched item value',refundMethod:'cash',authorizedByUserId:id.user,idempotencyKey:'refund-item-mismatch',itemsToRestock:[{productId:id.product,quantity:1}]}));
