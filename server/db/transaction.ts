...
export const createTransactionalPostgresExecutor = (pool: Pool): TransactionalSqlExecutor => ({
  query: <T extends QueryResultRow>(sql: string, parameters: readonly unknown[] = []) => pool.query<T>(sql, [...parameters]),
  transaction: async <T>(work: (tx: SqlQueryExecutor) => Promise<T>): Promise<T> => {
    const client: PoolClient = await pool.connect();
    try {
      await client.query('BEGIN');
      const result = await work({ query: <R extends QueryResultRow>(sql: string, parameters: readonly unknown[] = []) => client.query<R>(sql, [...parameters]) });
