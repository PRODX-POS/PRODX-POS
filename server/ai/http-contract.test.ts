...
function provider(onCall: () => void): AIProvider {
  return { name: 'openrouter', async chat(request) { onCall(); return { provider: 'openrouter', model: request.model, raw: {} }; } };
}
...
test('HTTP adapter derives authorization from verified principal and delegates to boundary', async () => {
  ...
  const response = await adapter.handle({ body: { messages: [{ role: 'user', content: 'hello' }] }, principal: { userId: 'user-1', organizationId: 'org-1', storeId: 'store-1', permissions: ['ai:use'] } }, boundary);
