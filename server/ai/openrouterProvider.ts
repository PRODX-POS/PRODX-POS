...
function normalizeBaseUrl(value: string): string {
  ...
  if (url.protocol !== 'https:') throw new Error('OpenRouter AI base URL must use HTTPS.');
...
export class OpenRouterProvider implements AIProvider {
  ...
  constructor(config: OpenRouterProviderConfig = {}) {
    ...
    this.baseUrl = normalizeBaseUrl(config.baseUrl ?? process.env.OPENROUTER_BASE_URL ?? DEFAULT_BASE_URL);
    this.defaultModel = config.defaultModel ?? process.env.OPENROUTER_DEFAULT_MODEL ?? DEFAULT_MODEL;
    this.timeoutMs = Math.max(1_000, Number(config.timeoutMs ?? process.env.OPENROUTER_TIMEOUT_MS ?? DEFAULT_TIMEOUT_MS));
    ...
    if (!this.apiKey) throw new Error('OpenRouter AI is not configured: OPENROUTER_API_KEY is missing.');
  }
  ...
  async chat(request: AIChatRequest): Promise<AIChatResponse> {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`OpenRouter AI request failed (${response.status}).`);
  ...
  private validateRequest(request: AIChatRequest): void {
    ...
    if (request.stream) throw new Error('OpenRouter provider does not support streaming through this boundary yet.');
    if (request.temperature !== undefined && (!Number.isFinite(request.temperature) || request.temperature < 0 || request.temperature > 2)) throw new Error('AI temperature must be a finite number between 0 and 2.');
