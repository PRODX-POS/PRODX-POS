...
const authenticateRequest = async (request: Request): Promise<RequestPrincipal | null> => {
  ...
  return {
    ...
    storeId: (await authDb.query<{ store_id: string }>(
      `SELECT store_id FROM prodx_devices WHERE id = $1 AND organization_id = $2 AND status = 'active' LIMIT 1`,
      [session.deviceId, session.organizationId],
    ))[0]?.store_id ?? '',
  };
};
...
const authorizeRequest = async (context: { principal: RequestPrincipal }, permission: string): Promise<boolean> => {
  ...
  const rows = await authDb.query<{ allowed: boolean }>(
    `SELECT EXISTS (
       SELECT 1
         ...
         JOIN prodx_roles r ON r.id = ur.role_id AND r.organization_id = ur.organization_id
...
const server = app.listen(port, '0.0.0.0', () => {
  console.info(`[prodx-pos] HTTP server listening on 0.0.0.0:${port}`);
});
