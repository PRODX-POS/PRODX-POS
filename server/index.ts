  ...
  const rows = await authDb.query<{ allowed: boolean }>(
    `SELECT EXISTS (
       SELECT 1
         ...
         JOIN prodx_role_permissions rp ON rp.organization_id = ur.organization_id AND rp.role_id = ur.role_id
        ...
        WHERE ur.organization_id = $1 AND ur.user_id = $2 AND ur.store_id = $3
          AND ur.active = TRUE AND r.active = TRUE AND p.permission_key = $4
     ) AS allowed`,
