...
CI must provision PostgreSQL 16, apply every migration in lexical order, verify the migration head is singular, verify UTC timestamp behavior, verify numeric precision behavior, and run rollback/idempotency checks where the migration contract permits them.
