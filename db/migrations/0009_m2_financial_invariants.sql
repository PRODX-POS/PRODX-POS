...
BEGIN
  ...
  IF v_item_count <> v_order.total_items_count OR v_item_total <> v_order.grand_total_amount THEN
    RAISE EXCEPTION 'order financial invariant violated for %', v_order_id USING ERRCODE='23514';
  END IF;
  ...
  IF v_payment_total <> v_order.grand_total_amount THEN
    RAISE EXCEPTION 'order payment invariant violated for %', v_order_id USING ERRCODE='23514';
  END IF;
  IF v_order.net_subtotal_amount + v_order.total_tax_amount <> v_order.grand_total_amount THEN
    RAISE EXCEPTION 'order tax/subtotal invariant violated for %', v_order_id USING ERRCODE='23514';
  END IF;
  RETURN NULL;
END;
$$;
...
CREATE CONSTRAINT TRIGGER prodx_order_financial_invariants AFTER INSERT OR UPDATE ON prodx_orders DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION prodx_validate_order_financial_invariants();
DROP TRIGGER IF EXISTS prodx_order_item_financial_invariants ON prodx_order_items;
CREATE CONSTRAINT TRIGGER prodx_order_item_financial_invariants AFTER INSERT OR UPDATE OR DELETE ON prodx_order_items DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION prodx_validate_order_financial_invariants();
DROP TRIGGER IF EXISTS prodx_payment_financial_invariants ON prodx_payments;
CREATE CONSTRAINT TRIGGER prodx_payment_financial_invariants AFTER INSERT OR UPDATE OR DELETE ON prodx_payments DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION prodx_validate_order_financial_invariants();
DROP TRIGGER IF EXISTS prodx_payment_currency_invariant ON prodx_payments;
CREATE TRIGGER prodx_payment_currency_invariant BEFORE INSERT OR UPDATE ON prodx_payments FOR EACH ROW EXECUTE FUNCTION prodx_validate_payment_currency();
